...
* Les analyses cartographiques seules offrent un aperçu général de vos données. La cartographie de vos sources de données est très rapide car elle n'accède pas aux fichiers pour voir les données à l'intérieur.
* Les analyses *Cartographier et classer* fournissent une analyse approfondie de vos données.


Vous pourriez tirer profit de l'utilisation d'analyses Map-only pour analyser rapidement vos données afin d'identifier les sources de données qui nécessitent des recherches plus approfondies, puis activer ultérieurement les analyses Map et de classification uniquement sur ces sources de données ou volumes requis.

La priorité est attribuée uniquement aux analyses de type « Map only », et non aux analyses de type « Map and classify ».
