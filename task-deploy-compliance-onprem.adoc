...
summary: 'Pour installer NetApp Data Classification sur un hôte Linux de votre réseau ou sur un hôte Linux dans le cloud disposant d"un accès Internet, vous devez déployer manuellement l"hôte Linux sur votre réseau ou dans le cloud.' 
---
...
Dans la plupart des cas, vous installez Data Classification sur un système à hôte unique. Il ne s'agit pas du même hôte VM que l'agent Console. <<Installation sur un seul hôte pour les configurations typiques,Voir ces étapes ici>>.
...
.Étapes
. Téléchargez le logiciel Data Classification depuis le https://mysupport.netapp.com/site/products/all/details/cloud-data-sense/downloads-tab/["Site de support NetApp"^]. Sélectionnez le fichier nommé *DATASENSE-INSTALLER-<version>.tar.gz*.
