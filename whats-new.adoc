...
Pour plus d'informations, consultez link:https://docs.netapp.com/us-en/data-services-data-classification/task-health-monitor.html["Surveiller l'état de la classification des données"^].
...
Cette version de classification BlueXP inclut des modifications de code qui optimisent l'utilisation des ressources.
...
Cette version inclut des améliorations mineures.

La version a également supprimé les options suivantes :

* L'option permettant d'activer la collecte des journaux d'audit a été désactivée.
* Lors de l'enquête sur les annuaires, l'option permettant de calculer le nombre de données d'informations personnelles identifiables (PII) par annuaires n'est pas disponible. Voir link:task-investigate-data.html["Examinez les données stockées dans votre organisation"].
...
Les versions 8 et 9 de Red Hat Enterprise Linux ne prennent pas en charge le moteur Docker, qui était auparavant requis pour l'installation de BlueXP classification. BlueXP classification est désormais compatible avec RHEL 9.0, 9.1 et 9.2 en utilisant Podman version 4 ou supérieure comme infrastructure de conteneurs. Si votre environnement nécessite l'utilisation des versions les plus récentes de RHEL, vous pouvez désormais installer BlueXP classification (version 1.26 ou supérieure) avec Podman.

Actuellement, Data Classification ne prend en charge ni les installations de sites isolés ni les environnements de scan distribués (utilisant un nœud maître et des nœuds de scan distants) lors de l'utilisation de RHEL 9.x.
...
=== Ajoutez des mots clés personnalisés depuis l'interface utilisateur de classification BlueXP
...
Le filtre « Nombre d'utilisateurs ayant accès » prend désormais en charge les fichiers stockés dans les référentiels SharePoint Online. Auparavant, seuls les fichiers sur les partages CIFS étaient pris en charge. Les groupes SharePoint qui ne sont pas basés sur Active Directory ne sont pas comptabilisés dans ce filtre pour le moment.
