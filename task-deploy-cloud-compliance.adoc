...
Notez que vous pouvez également link:task-deploy-compliance-onprem.html["installer Data Classification sur un hôte Linux disposant d'un accès Internet"]. Ce type d'installation peut être une bonne option si vous préférez analyser des systèmes ONTAP sur site à l'aide d'une instance de NetApp Data Classification également située sur site, mais cela n'est pas une obligation. Le logiciel fonctionne exactement de la même manière, quelle que soit la méthode d'installation que vous choisissez.
...
Si vous n’avez pas encore d’agent de console, créez-en un. Voirlink:https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-aws.html["création d'un agent de console dans AWS"^] ,link:https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-azure.html["création d'un agent de console dans Azure"^] , oulink:https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-google.html["création d'un agent de console dans GCP"^] .

[role="quick-margin-para"]
Vous pouvez égalementlink:https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html["installer l'agent de console sur site"^] sur un serveur Linux de votre réseau ou sur un serveur Linux dans le cloud.

.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-2.png["Deux"]Prérequis
[role="quick-margin-para"]
Assurez-vous que votre environnement répond aux prérequis. Cela inclut un accès Internet sortant pour l'instance, une connectivité entre l'agent Console et Data Classification via le port 443, et plus encore. <<Prérequis,Voir la liste complète>>.
...
Si vous n'avez pas encore d'agent Console, créez un agent Console auprès de votre fournisseur de cloud. Voir  https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-aws.html["création d'un agent de console dans AWS"^] ou  https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-azure.html["création d'un agent de console dans Azure"^], ou  https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-google.html["création d'un agent de console dans GCP"^]. Dans la plupart des cas, vous avez probablement déjà un agent Console configuré avant de tenter d'activer NetApp Data Classification, car la plupart  https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html["Les fonctionnalités de la console nécessitent un agent de console"], mais il existe des cas où vous devez en configurer un maintenant.

Il existe certains scénarios où vous devez utiliser un agent de Console déployé chez un fournisseur de cloud :
...
Notez que vous pouvez également https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html["installer l'agent de console sur site"^] sur un serveur Linux de votre réseau ou dans le cloud. Certains utilisateurs prévoyant d’installer la classification des données sur site peuvent également choisir d’installer l’agent de console sur site.

Il peut y avoir des situations où vous devez utiliser https://docs.netapp.com/us-en/console-setup-admin/concept-agents.html#multiple-systems-and-agents["plusieurs agents de console"^] .
...
link:https://docs.netapp.com/us-en/console-setup-admin/task-install-restricted-mode.html["Découvrez comment déployer l'agent Console dans une région gouvernementale"^].
...
* link:https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-resource-limits.html["Documentation AWS : quotas de service Amazon EC2"^]
* link:https://docs.microsoft.com/en-us/azure/virtual-machines/linux/quotas["Documentation Azure : Quotas de processeurs virtuels pour machines virtuelles"^]
* link:https://docs.cloud.google.com/docs/quotas/view-manage["Documentation Google Cloud : quotas de ressources"^]
