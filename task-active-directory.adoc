...
---
= Intégrer Active Directory à NetApp Data Classification
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* Vous pouvez consulter link:task-investigate-data.html["autorisations complètes du fichier"] pour chaque fichier et répertoire lorsque vous sélectionnez le bouton « Afficher toutes les autorisations ».
...
Pour les volumes CIFS et les partages de fichiers, afin de garantir que la date et l'heure du dernier accès aux fichiers restent inchangées après les analyses de classification des données, l'utilisateur doit disposer de l'autorisation d'écriture des attributs. Dans la mesure du possible, il est recommandé d'intégrer l'utilisateur configuré dans Active Directory à un groupe parent de l'organisation disposant des autorisations d'accès à tous les fichiers.
...
. Dans l'en-tête Active Directory, sélectionnez **Ajouter**.
. Dans la boîte de dialogue Se connecter à Active Directory, saisissez les informations d'Active Directory, puis sélectionnez *Connecter*.
+
...
Si vous devez modifier des valeurs dans votre intégration Active Directory, vous devez mettre à jour la configuration Active Directory dans Data Classification. Retournez au menu **Configuration** dans Data Classification puis accédez à l’onglet Paramètres pour effectuer les modifications.

Vous pouvez également supprimer l'intégration en sélectionnant le bouton **Supprimer** sous Active Directory.
