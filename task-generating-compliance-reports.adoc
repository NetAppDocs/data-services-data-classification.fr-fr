...
---
= Consultez les rapports de conformité dans NetApp Data Classification
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[NOTE]
====
* Les rapports de conformité ne sont disponibles que si vous effectuez une analyse de classification complète de vos sources de données. Les sources de données ayant fait l'objet d'une simple cartographie ne peuvent générer que le rapport de cartographie des données.
