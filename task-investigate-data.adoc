...
---
= Analysez les données de votre organisation grâce à NetApp Data Classification
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Le tableau de bord « Analyse des données » affiche des informations détaillées sur vos données au niveau des fichiers et des répertoires, vous permettant de trier et de filtrer les résultats. La page « Analyse des données » présente des informations sur les métadonnées et les permissions des fichiers et des répertoires ainsi que l’identification des fichiers en double. Grâce à des informations au niveau des fichiers, des répertoires et des bases de données, vous pouvez prendre des mesures pour améliorer la conformité de votre organisation et économiser de l’espace de stockage. La page « Analyse des données » prend également en charge link:task-copy-move.html["déplacement, copie et suppression de fichiers"].


NOTE: Pour exploiter les informations de la page « Investigation », vous devez effectuer une analyse de classification complète de vos sources de données. Les sources de données ayant fait l'objet d'une analyse cartographique uniquement n'affichent pas les détails au niveau du fichier.
...
| Nombre d'identifiants | Sélectionnez la plage d'identifiants sensibles détectés par fichier. Inclut les données personnelles et les données personnelles sensibles. Lors du filtrage dans les répertoires, NetApp Data Classification totalise les correspondances de tous les fichiers de chaque dossier (et sous-dossiers). REMARQUE : La version de décembre 2023 (version 1.26.6) a supprimé l'option permettant de calculer le nombre de données à caractère personnel identifiables (PII) par répertoires. 
...
TIP: Après le filtrage des données, vous pouvez link:task-copy-move.html["copier, déplacer et supprimer des fichiers non structurés"] pour améliorer la conformité ou optimiser le stockage.
...
* link:task-copy-move.html["Copier, déplacer et supprimer des données depuis le tableau de bord d'enquête"]
