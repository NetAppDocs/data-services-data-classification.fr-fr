...
+
** NetApp Data Classification ne peut pas extraire les autorisations ni la « dernière date d'accès » des systèmes 7-Mode.
...
NOTE: La page de présentation du système comporte une option *Analyser sans autorisation d'écriture* désactivée par défaut. Si NetApp Data Classification ne dispose pas des autorisations appropriées, elle n'analysera pas les fichiers, car NetApp Data Classification ne peut pas rétablir la « dernière date d'accès » à l'horodatage d'origine. Vous pouvez remplacer ce paramètre à l'aide de l'option *Analyser sans autorisation d'écriture*. Pour en savoir plus sur cette option, consultez link:reference-collected-metadata.html#last-accessed-time-timestamp["Métadonnées collectées à partir de sources de données dans la classification des données"].
