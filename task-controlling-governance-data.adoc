...
summary: La classification des données permet de réduire les coûts de stockage en identifiant les données obsolètes, les fichiers dupliqués et les fichiers volumineux, puis en les supprimant ou en les déplaçant vers un stockage objet moins coûteux. 
---
= Consultez les détails de gouvernance relatifs aux données organisationnelles grâce à NetApp Data Classification
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
.Étapes
. Dans le menu de la NetApp Console , sélectionnez *Gouvernance > Classification*.
...
. Dans chaque vignette « Opportunités d'économies » du tableau de bord Gouvernance, sélectionnez *Optimiser le stockage* pour afficher les résultats filtrés sur la page Investigation. Pour découvrir les données que vous devriez supprimer ou transférer vers un stockage moins coûteux, examinez les _Saving Opportunities_.
+
...
.Étapes
. Dans Classification des données, sélectionnez *Gouvernance*.
. Dans la section « Rapports », sélectionnez le bouton **Télécharger** sous l’en-tête du rapport « Aperçu complet de la cartographie des données ».
+
...
La zone _Autorisations d'ouverture_ du rapport Présentation du mappage des données affiche le pourcentage pour chaque type d'autorisations qui existent pour tous les fichiers en cours d'analyse.  Le graphique montre les types d’autorisations suivants :
