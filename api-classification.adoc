...
summary: Les fonctionnalités de NetApp Data Classification disponibles via l’interface utilisateur Web sont également disponibles via l’API. 
---
...
[role="lead"]
Utilisez l’API REST de classification des données NetApp pour accéder aux mêmes fonctionnalités que celles disponibles dans l’interface utilisateur web.
...
** Les données sont exportées au format JSON (facile à analyser et à transférer vers des applications tierces, telles que Splunk)


* Créez des requêtes à l'aide d'instructions « AND » et « OR », incluez et excluez des informations, et bien plus encore.
+
Par exemple, vous pouvez localiser des fichiers _sans_ informations personnelles identifiables (PII) (fonctionnalité non disponible dans l'interface utilisateur). Vous pouvez également exclure certains champs de l'opération d'exportation.
...
Pour accéder à Swagger, vous avez besoin de l'adresse IP de votre instance NetApp Data Classification. Pour un déploiement cloud, utilisez l'adresse IP publique. Vous devez ensuite accéder à cet endpoint :
...
.Demande d'API
Vous devez obtenir tous les champs et options pertinents pour qu’un système puisse afficher tous les filtres de l’onglet Investigation.
...
----
Utilisez la réponse dans les paramètres de la requête pour filtrer les fichiers que vous souhaitez copier.
...
Créer l'action de copie :

.Demande d'API
Cette API vous permet de créer plusieurs actions.
