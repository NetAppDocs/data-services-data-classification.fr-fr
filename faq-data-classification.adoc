...
---
= FAQ sur la NetApp Data Classification
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utilisez cette FAQ pour trouver rapidement des réponses aux questions courantes sur NetApp Data Classification.
...
Data Classification ajoute une couche supplémentaire d'IA à votre système NetApp Console et à vos systèmes de stockage. Elle analyse ensuite les données sur les volumes, les compartiments, les bases de données et autres comptes de stockage, puis indexe les informations sur les données qu'elle trouve. Data Classification exploite à la fois l'intelligence artificielle et le traitement automatique du langage naturel, contrairement aux solutions alternatives qui sont généralement basées sur les expressions régulières et la correspondance de modèles.

La classification des données utilise l'IA pour fournir une compréhension contextuelle des données, permettant une détection et une classification précises. Elle est pilotée par l'IA car elle est conçue pour les types et volumes de données modernes. Elle comprend également le contexte des données afin d'offrir une découverte et une classification solides et précises.
...
La classification des données fait partie des fonctionnalités principales de la NetApp Console. Vous n'avez pas besoin de déployer la classification des données depuis une place de marché cloud.
...
Les performances d'analyse peuvent varier en fonction de la bande passante du réseau et de la taille moyenne des fichiers dans votre environnement. Elles peuvent également dépendre de la taille et de l'emplacement du système hôte (c'est-à-dire, s'il se trouve dans le cloud ou sur site). Voir link:concept-classification.html["L'instance de classification des données"] et link:task-deploy-overview.html["Déploiement de la classification des données"] pour plus d'informations.

Lors de l'ajout initial de nouvelles sources de données, vous pouvez également choisir d'effectuer uniquement une analyse de mappage ou une analyse complète. Le mappage peut être effectué sur vos sources de données très rapidement car il n'accède pas aux fichiers pour voir les données à l'intérieur. link:task-scanning-overview.html["Comprendre la différence entre une analyse de cartographie et une analyse de classification"].
...
=== Si vous redémarrez votre machine virtuelle pendant les analyses, quel sera l'impact sur l'analyse ?

Les analyses persistent. Si vous redémarrez une machine virtuelle pendant l'exécution d'analyses, celles-ci reprennent là où elles s'étaient arrêtées après le redémarrage réussi de la machine virtuelle.
...
Pour supprimer complètement l'instance de NetApp Data Classification, supprimez manuellement l'instance de NetApp Data Classification depuis le portail de votre fournisseur de cloud ou depuis votre emplacement sur site.
...
Lorsque la classification des données analyse des volumes contenant des données froides stockées dans un stockage objet à l'aide d'analyses uniquement cartographiques, elle analyse toutes les données : celles présentes sur les disques locaux et les données froides stockées dans un stockage objet. Ceci est également valable pour les produits non-NetApp qui implémentent la hiérarchisation.

L'analyse cartographique seule ne réchauffe pas les données froides : elles restent froides et demeurent dans le stockage d'objets. En revanche, si vous effectuez l'analyse complète, certaines configurations peuvent réchauffer les données froides.
...
Lorsque la classification des données détecte des informations personnelles identifiables (IPI), ou lorsqu'elle effectue une recherche DSAR, seuls les formats de fichiers suivants sont pris en charge :
...
La classification des données vous permet d'effectuer une analyse cartographique uniquement ou une analyse complète de vos sources de données. Les analyses cartographiques uniquement offrent un aperçu général de vos données, tandis que les analyses complètes permettent une exploration approfondie de vos données. Les analyses cartographiques uniquement peuvent être effectuées très rapidement sur vos sources de données car elles n'accèdent pas aux fichiers pour voir les données à l'intérieur.
...
* *Analyse approfondie de la classification des données (analyse complète)* : Data Classification analyse les données à l'aide de protocoles standard et d'autorisations en lecture seule dans l'ensemble de vos environnements. Certains fichiers sont ouverts et analysés pour détecter des données sensibles liées à l’activité de l’entreprise, des informations privées et des problèmes liés aux ransomwares.
+
...
La classification des données capture des métadonnées telles que : nom du fichier, permissions, date de création, date du dernier accès et date de la dernière modification. Cela inclut toutes les métadonnées qui apparaissent sur la page Détails de l’analyse des données et dans les rapports d’analyse des données.
...
Non. Les données privées échangées entre votre navigateur et l'instance de Data Classification sont sécurisées par un chiffrement de bout en bout utilisant TLS 1.2, ce qui signifie que NetApp et des tiers ne peuvent pas les lire. Data Classification ne partage aucune donnée ni aucun résultat avec NetApp sauf si vous en faites la demande et approuvez l'accès.
...
Lors de l'analyse des données, l'agent NetApp Console doit être installé aux emplacements suivants :
...
La NetApp Console vous permet d’analyser et de générer des rapports sur les systèmes, quel que soit leur emplacement, y compris sur site, dans le cloud ou dans des environnements hybrides. NetApp Data Classification est généralement déployé selon un modèle SaaS, dans lequel le service est activé via l’interface de la Console et ne nécessite aucune installation matérielle ou logicielle. Même dans ce mode de déploiement « click-and-run », la gestion des données peut être effectuée, que les magasins de données soient sur site ou dans le cloud public.
...
* Dans AWS, la classification des données s'exécute sur une instance m6i.4xlarge avec un disque GP2 de 500 GiB.
