...
---
= Installez NetApp Data Classification sur un hôte Linux en mode privé
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utilisez le mode privé pour installer NetApp Data Classification sur un hôte Linux dans un site sur site sans accès Internet. Ce déploiement utilise un script d'installation et maintient l'instance Data Classification déconnectée de la couche SaaS de la NetApp Console, vous permettant d'analyser des sources de données locales dans un environnement isolé.
...
Le script d'installation de NetApp Data Classification vérifie si le système et l'environnement répondent aux prérequis requis. Si c'est le cas, l'installation démarre. Si vous souhaitez vérifier les prérequis indépendamment de l'exécution de l'installation de NetApp Data Classification, vous pouvez télécharger un package logiciel distinct qui teste uniquement les prérequis. link:task-test-linux-system.html["Découvrez comment vérifier si votre hôte Linux est prêt à installer la classification des données"].
...
* La classification des données doit être exécutée sur un serveur dédié. Le serveur ne peut pas être partagé avec d'autres applications ou des logiciels tiers tels qu'un antivirus.
...
* *Gestion des abonnements Red Hat* : L’hôte doit être enregistré auprès de Red Hat Subscription Management. S’il n’est pas enregistré, le système ne peut pas accéder aux dépôts pour mettre à jour les logiciels tiers requis lors de l’installation.
...
Dans la plupart des cas, vous installez Data Classification sur un système à hôte unique. Ce n'est pas le même hôte de machine virtuelle que l'agent Console.
